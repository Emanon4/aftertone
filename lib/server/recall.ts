...
/** The whole local index participates in recall; only this bounded pool goes to Jev. */
export function selectCandidatePool(seed:Track,library:Track[],live:Track[],excluded:string[],direction="close",limit=200,random:()=>number=Math.random):Track[]{
 ...
 const take=(pool:Track[],count:number,source?:string)=>{
  let taken=0;for(const t of pool){if(taken>=count||result.length>=limit)break;const a=clean(t.artist);
   if((!t.preview&&!t.previewAvailable)||seenKeys.has(key(t))||seenRecordings.has(recording(t))||(t.isrc&&seenISRCs.has(t.isrc))||(artists.get(a)||0)>=3||(direction!=="close"&&a===seedArtist))continue;
 ...
 const quotas=direction==="bold"?[40,70,40]:direction==="sideways"?[60,80,40]:[80,80,20];
