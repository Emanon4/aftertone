...
export default function Home(){
 ...
 return <main className={`site-shell ${current?"has-player-track":""}`}><Tabs value={view} onValueChange={setView}>
  ...
  <TabsContent value="discover">
   <section className={`hero ${seed?"has-seed":""}`}><p className="eyebrow"><span/> A LITTLE CLOSER TO YOUR NEXT FAVORITE</p><h1>下一首，<br/><span>你的单曲循环。</span><i aria-hidden="true">✳</i></h1><p className="hero-description">世界很大，好歌很多。<br className="mobile-break"/> 从喜欢的一首，听向还没遇见的世界。</p><p className="hero-jev"><span>Jev</span> 辅助筛选，每轮从最多 <strong>5,000</strong> 首候选中，发现你的下一组七首。</p><button className="current-model" onClick={()=>setModelSettingsOpen(true)}><span>当前：{modelSettingsSummary(modelSettings)}{previewOnly?" · 需连接筛选服务":""}</span><span>模型设置<ArrowUpRight size={12}/></span></button></section>
   ...
   <section className="listening-room"><div className="section-heading"><div><p className="eyebrow">{recommendations===null?"THE LISTENING ROOM":"YOUR NEXT SEVEN"}</p><h2>{recommendations===null?"从这里，听见别处。":"七首歌，新的心动。"}</h2></div><span className="section-note">{recommendations===null?(previewOnly?"点击封面，在音乐平台收听":"点击封面，试听约 30 秒"):`第 ${String(round+1).padStart(2,"0")} 组 · ${visible.length} 首`}</span></div>
 ...
 <footer className="page-footer"><span>Music is a place to go.</span><div className="footer-links"><ThemeToggle/><ModelSettingsDialog settings={modelSettings} open={modelSettingsOpen} onOpenChange={setModelSettingsOpen} busy={busy} previewOnly={previewOnly} byokAvailable={byokAvailable} onApply={settings=>{setModelSettings(settings);setError("");setToast(`已应用 ${modelSettingsSummary(settings)}${previewOnly?" · 需连接筛选服务":""}`);}}/><Dialog><DialogTrigger className="about-link">关于余音<Info size={13}/></DialogTrigger><DialogContent className="about-dialog"><DialogTitle>让下一次偶遇，更有方向。</DialogTitle><DialogDescription>余音 · Aftertone</DialogDescription><p>从你喜欢的一首歌出发，召回最多 5,000 首真实候选，由 Jev 根据歌曲资料与明确偏好筛选，七首一组呈现。</p><p>Jev 不能直接听音频。资料关联是试听线索，不是对“好听”的保证；曲库中的策展集合也不等于官方音乐风格。</p><p>当前索引包含 {library.tracks.toLocaleString()} 首，仍偏向艺人热门作品，不覆盖全球音乐。曲目资料和试听片段来自 Deezer，部分华语搜索由 iTunes 补充。站内提供短试听；点击“听完整首”可选择 Deezer、Apple Music、Spotify、网易云、QQ 音乐或 YouTube Music；已有来源链接直达歌曲，其他平台按歌名和歌手搜索，完整版能否播放由平台账号、订阅与所在地区决定。</p><p>收藏和不合适记录保存在当前浏览器。筛选会将歌曲资料、偏好和最近最多 8 条正负反馈发给当前所选模型服务。站点 Jev 全站每天最多 30 轮；个人 Key 独立每日 30 轮。取消后已在途的模型调用可能仍会完成。</p><a href="https://docs.typesafe.ai/concepts/state" target="_blank" rel="noreferrer">了解 Jev 的能力<ArrowUpRight size={14}/></a>{dismissed.length>0&&<button className="text-button" onClick={()=>{setDismissed([]);setToast("已清空不合适记录");}}>重置不合适记录<RotateCcw size={14}/></button>}</DialogContent></Dialog></div><span>MADE FOR THE REPLAY</span></footer>
 ...
 <div className={`player-bar glass ${current?"has-track":""}`}>
  {current?<>
   ...
   <div className="player-idle"><span className="idle-icon"><Headphones size={21}/></span><span>给下一首好歌，<strong>一点时间。</strong></span></div><button className="idle-play" onClick={()=>play(focused||featured[0])} aria-label={previewOnly?`去 ${providerName(focused||featured[0])} 听《${(focused||featured[0]).title}》完整版（新窗口）`:"播放试听片段"}>{previewOnly?"听完整首":"试听片段"}<Play size={15} fill="currentColor"/></button><span className="player-caption">{previewOnly?"去音乐平台，听完整首。":"试听片段 · 约 30 秒"}</span>
  </>}
 </div>
