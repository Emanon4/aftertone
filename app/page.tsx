...
import {ArrowUpRight,ArrowRight,AudioLines,Heart,Play,Pause,Search,X,LoaderCircle,RotateCcw,Info,Check,Volume2,Disc3,Headphones} from "lucide-react";
...
async function parseResponse(response:Response):Promise<ApiData>{const type=response.headers.get("content-type")||"";if(!type.includes("json"))throw new Error("筛选服务暂未连接，稍后再试。你仍可以打开音乐平台收听。");const data=await response.json() as ApiData;if(!response.ok)throw new Error(data.error||"音乐服务暂时不可用。");return data;}
export default function Home(){
 ...
 const [current,setCurrent]=useState<Track|null>(null),[playing,setPlaying]=useState(false),[loadingTrack,setLoadingTrack]=useState(""),[position,setPosition]=useState(0),[duration,setDuration]=useState(30),[volume,setVolume]=useState(.7),[focus,setFocus]=useState(()=>typeof window!=="undefined"&&window.innerWidth<760?0:3);
 ...
 useEffect(()=>{if(previewOnly)return;const controller=new AbortController();fetch(api("/api/library"),{signal:controller.signal}).then(r=>r.ok?r.json():null).then(value=>{const d=value as typeof library|null;if(d?.tracks)setLibrary(d);}).catch(()=>{});return()=>controller.abort();},[]);
 ...
 async function recommend(excludePrevious=false){
  if(previewOnly){setError("智能找歌即将开放，请先在音乐平台收听。");return;}
  if(!seed)return;
  ...
  let ownJob:string|null=null;
  try{
   let data=await parseResponse(await fetch(api("/api/recommend"),{method:"POST",headers:{"Content-Type":"application/json"},signal:c.signal,body:JSON.stringify({seed:{id:seed.id,provider:seed.provider},direction,notes,excluded:[...new Set(excluded)].slice(0,150),feedback:{liked:saved.slice(0,8).map(x=>`${x.artist} — ${x.title}`),disliked:dismissed.slice(0,8).map(x=>`${x.artist} — ${x.title}`)}})}));
   ...
   while(ownJob&&data.status!=="done"&&data.status!=="complete"){
    ...
    try{
     if(data.status==="running"){
      ...
      data=await parseResponse(await fetch(jobUrl+"/step",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({step:data.nextStep}),signal:c.signal}));
     }
    }catch(stepError){
     if(!active())return;
     // Recover committed work by reading its state before advancing another step.
     try{data=await parseResponse(await fetch(jobUrl,{signal:c.signal}));}
 ...
 async function play(t:Track){if(previewOnly){window.open(t.url,"_blank","noopener,noreferrer");return;}const audio=audioRef.current;if(!audio)return;if(current&&trackKey(current)===trackKey(t)&&audio.src){if(!audio.paused){audio.pause();return;}try{await audio.play();}catch{setError("请再次点击试听。");}return;}const version=++playbackId.current;audio.pause();audio.removeAttribute("src");audio.load();setCurrent(t);setPosition(0);setDuration(30);setLoadingTrack(trackKey(t));setError("");try{const data=await parseResponse(await fetch(api(`/api/music?id=${t.id}&provider=${t.provider}`)));if(version!==playbackId.current)return;if(!data.track.preview)throw new Error("这首暂时没有试听，可打开音乐平台听完整版。");setCurrent({...t,...data.track});audio.src=data.track.preview;audio.volume=volume;await audio.play();}catch(e){if(version===playbackId.current)setError(e instanceof Error?e.message:"试听暂不可用。");}finally{if(version===playbackId.current)setLoadingTrack("");}}
 ...
 return <main className="site-shell"><Tabs value={view} onValueChange={setView}>
  <header className="masthead"><a className="wordmark" href={base} aria-label="余音首页"><AudioLines size={27}/><span>aftertone<span className="brand-cn">余音</span></span></a><TabsList className="site-nav"><TabsTrigger value="discover">发现音乐</TabsTrigger><TabsTrigger value="saved">我的收藏 <span>{saved.length.toString().padStart(2,"0")}</span></TabsTrigger></TabsList><span className="header-note"><span/>为好音乐，留一点时间</span></header>
  <TabsContent value="discover">
   <section className={`hero ${seed?"has-seed":""}`}><p className="eyebrow"><span/> A LITTLE CLOSER TO YOUR NEXT FAVORITE</p><h1>下一首，<br/><span>你的单曲循环。</span><i aria-hidden="true">✳</i></h1><p className="hero-description">世界很大，好歌很多。<br className="mobile-break"/> 从喜欢的一首，听向还没遇见的世界。</p></section>
   <section className="discovery-console">
    <form className="search-form glass" onSubmit={e=>{e.preventDefault();search();}}><Search size={21}/><input ref={searchInput} aria-label="搜索喜欢的歌" placeholder="你最近反复听的，是哪首歌？" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==="Escape")setSearchResults(null);}} maxLength={120}/><button className="primary-button" type="submit" disabled={searching}>{searching?<LoaderCircle className="spin" size={17}/>:<>找一首歌<ArrowUpRight size={17}/></>}</button></form>
    {previewOnly&&<p className="service-note">听音室预览 · 智能找歌与站内试听即将开放，当前可前往音乐平台收听。</p>}
    {!seed&&<div className="search-hints"><span>也可以从这些声音开始</span>{["Men I Trust","Radiohead","周杰伦"].map(q=><button key={q} onClick={()=>search(q)}>{q}<ArrowUpRight size={12}/></button>)}</div>}
   ...
   <section className="listening-room"><div className="section-heading"><div><p className="eyebrow">{recommendations===null?"THE LISTENING ROOM":"YOUR NEXT SEVEN"}</p><h2>{recommendations===null?"从这里，听见别处。":"七首歌，新的心动。"}</h2></div><span className="section-note">{recommendations===null?(previewOnly?"点击封面，在音乐平台收听":"点击封面，先听一小段"):`第 ${String(round+1).padStart(2,"0")} 组 · ${visible.length} 首`}</span></div>
    {busy&&<div className="finding glass" role="status"><div className="finding-label"><AudioLines size={20}/><strong>{progress?`已筛选 ${progress.scoredCount.toLocaleString()} / ${progress.totalCount.toLocaleString()} 首`:"正在为你整理 5,000 首候选"}</strong><span>{progress?`${percent}%`:"准备中"}</span><button onClick={cancel}>取消</button></div><div className={`finding-track ${!progress?"indeterminate":""}`}><span style={{width:`${percent}%`}}/></div><p>{progress?"让更多可能，被你听见。":"从曲库与关联艺术家中召回，筛选数量以实际合格候选为准。"}</p></div>}
    {visible.length>0?<><div className={`record-shelf ${busy?"is-loading":""}`} ref={shelfRef}>{visible.map((t,i)=>{const active=current&&trackKey(current)===trackKey(t);return <article className={`record ${focus===i?"is-focus":""} ${active&&playing?"is-current":""}`} style={{"--offset":Math.abs(i-3),"--tilt":`${(i-3)*1.25}deg`,"--index":i} as CSSProperties} key={trackKey(t)}><div className="cover-wrap"><button className="cover-art" onClick={()=>{setFocus(i);play(t);}} aria-label={`${previewOnly?"在音乐平台收听":active&&playing?"暂停":"试听"} ${t.title}`} disabled={loadingTrack===trackKey(t)}><img src={t.image} alt={`${t.album} 专辑封面`} loading={i<4?"eager":"lazy"}/><span className="cover-play">{loadingTrack===trackKey(t)?<LoaderCircle className="spin" size={19}/>:active&&playing?<Pause size={19} fill="currentColor"/>:<Play size={19} fill="currentColor"/>}</span></button><button className={`cover-save ${isSaved(t)?"is-saved":""}`} onClick={()=>save(t)} aria-label={`${isSaved(t)?"取消收藏":"收藏"} ${t.title}`} aria-pressed={isSaved(t)}><Heart size={16} fill={isSaved(t)?"currentColor":"none"}/></button>{active&&playing&&<span className="now-playing"><AudioLines size={15}/>正在试听</span>}</div><button className="record-name" onClick={()=>setFocus(i)}><span className="record-number">{String(i+1).padStart(2,"0")}</span><span><strong>{t.title}</strong><small>{t.artist}</small></span></button></article>;})}</div><div className="shelf-navigation" aria-label="选择曲目">{visible.map((t,i)=><button key={trackKey(t)} aria-label={`查看第 ${i+1} 首 ${t.title}`} aria-pressed={focus===i} onClick={()=>focusTrack(i)}>{String(i+1).padStart(2,"0")}</button>)}</div>{focused&&<div className="focus-detail"><div className="detail-identity"><span className="detail-index">{String(Math.min(focus,visible.length-1)+1).padStart(2,"0")}<i>/ {String(visible.length).padStart(2,"0")}</i></span><div><h3>{focused.title}</h3><p>{focused.reason||`${focused.artist} · ${focused.album}`}</p></div></div><div className="detail-actions"><button className="text-button" onClick={()=>choose(focused)}>{recommendations===null?"从这首出发":"沿着这首继续"}<ArrowRight size={16}/></button><a href={focused.url} target="_blank" rel="noreferrer">{providerName(focused)}<ArrowUpRight size={15}/></a>{recommendations!==null&&<button onClick={()=>dismiss(focused)}>不太合适</button>}</div>{focused.provider==="itunes"&&<p className="attribution">试听 provided courtesy of iTunes</p>}</div>}</>:<div className="empty-state"><Disc3 size={34}/><h3>这一轮，没有勉强凑数。</h3><p>换一个起点，或者放宽刚才的条件。</p><button className="text-button" onClick={()=>{setRecommendations(null);setNotes("");}}>重新找个起点<ArrowRight size={16}/></button></div>}
    <div className="collection-line"><span><span className="status-dot"/>{library.tracks.toLocaleString("en-US")} 首已收录<span className="collection-artists"> · {library.artists.toLocaleString("en-US")} 位艺术家</span></span>{recommendations!==null?<button className="text-button" onClick={next} disabled={busy}>{(round+1)*7<(recommendations?.length||0)?"再听七首":"寻找新一轮"}<ArrowRight size={16}/></button>:<span>每轮筛选目标 <strong>5,000</strong> 首</span>}</div>{meta&&<p className="round-meta">本轮实际筛选 {meta.candidateCount||progress?.totalCount||0} 首 · {((meta.wallMs||meta.elapsedMs||progress?.elapsedMs||0)/1000).toFixed(1)} 秒 · {meta.model||"Jev"}</p>}
   </section>
  </TabsContent>
  <TabsContent value="saved"><section className="saved-heading"><p className="eyebrow">THE ONES THAT STAY</p><h1>留下来的声音<span>{saved.length.toString().padStart(2,"0")}</span></h1><p>让你点下爱心的歌，都在这里。</p></section>{saved.length?<div className="saved-list">{saved.map((t,i)=><div className="saved-row" key={trackKey(t)}><span className="saved-number">{String(i+1).padStart(2,"0")}</span><button className="saved-cover" onClick={()=>play(t)} aria-label={`试听收藏 ${t.title}`}><img src={t.image} alt=""/><Play size={17}/></button><div className="saved-info"><h3>{t.title}</h3><p>{t.artist}</p></div><span className="saved-album">{t.album}</span><button className="text-button" onClick={()=>choose(t)}>沿着这首找<ArrowRight size={15}/></button><button className="heart-button is-saved" onClick={()=>save(t)} aria-label={`取消收藏 ${t.title}`}><Heart fill="currentColor" size={18}/></button></div>)}</div>:<div className="empty-state"><Heart size={34}/><h3>还没有留下的歌</h3><p>听到心动的那一首，点一下爱心。</p><button className="text-button" onClick={()=>setView("discover")}>去听点新的<ArrowRight size={16}/></button></div>}</TabsContent>
 </Tabs>
 <footer className="page-footer"><span>Music is a place to go.</span><Dialog><DialogTrigger className="about-link">关于余音<Info size={13}/></DialogTrigger><DialogContent className="about-dialog"><DialogTitle>让下一次偶遇，更有方向。</DialogTitle><DialogDescription>余音 · Aftertone</DialogDescription><p>从你喜欢的一首歌出发，召回最多 5,000 首真实候选，由 Jev 根据歌曲资料与明确偏好筛选，七首一组呈现。</p><p>Jev 不能直接听音频。资料关联是试听线索，不是对“好听”的保证；曲库中的策展集合也不等于官方音乐风格。</p><p>当前索引包含 {library.tracks.toLocaleString()} 首，仍偏向艺人热门作品，不覆盖全球音乐。曲目资料和试听来自 Deezer，部分华语搜索由 iTunes 补充。完整歌曲请前往对应音乐平台收听。</p><p>收藏和不合适记录保存在当前浏览器。筛选会将歌曲资料、偏好和最近最多 8 条正负反馈发给 Jev。全站每天最多 30 轮，取消后已在途的模型调用可能仍会完成。</p><a href="https://docs.typesafe.ai/concepts/state" target="_blank" rel="noreferrer">了解 Jev 的能力<ArrowUpRight size={14}/></a>{dismissed.length>0&&<button className="text-button" onClick={()=>{setDismissed([]);setToast("已清空不合适记录");}}>重置不合适记录<RotateCcw size={14}/></button>}</DialogContent></Dialog><span>MADE FOR THE REPLAY</span></footer>
 <audio ref={audioRef} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onEnded={()=>setPlaying(false)} onTimeUpdate={e=>setPosition(e.currentTarget.currentTime)} onLoadedMetadata={e=>setDuration(Number.isFinite(e.currentTarget.duration)?e.currentTarget.duration:30)} onError={()=>{if(audioRef.current?.getAttribute("src")){setPlaying(false);setError("试听暂时无法播放，可以打开音乐平台收听。");}}} preload="none"/>
 <div className={`player-bar glass ${current?"has-track":""}`}>{current?<><div className="player-track"><img src={current.image} alt=""/><div><strong>{current.title}</strong><span>{current.artist}</span></div><button className={`heart-button ${isSaved(current)?"is-saved":""}`} aria-label={`${isSaved(current)?"取消收藏":"收藏"}正在试听的歌曲`} onClick={()=>save(current)}><Heart size={17} fill={isSaved(current)?"currentColor":"none"}/></button></div><div className="transport"><button className="transport-play" onClick={()=>play(current)} aria-label={playing?"暂停试听":"继续试听"} disabled={!!loadingTrack}>{loadingTrack?<LoaderCircle className="spin" size={19}/>:playing?<Pause size={19} fill="currentColor"/>:<Play size={19} fill="currentColor"/>}</button><span className="time">{seconds(position)}</span><Slider aria-label="试听进度" value={[position]} max={duration||30} step={.1} onValueChange={([v])=>{if(audioRef.current&&audioRef.current.readyState>0){audioRef.current.currentTime=v;setPosition(v);}}}/><span className="time">{seconds(duration)}</span></div><div className="player-extras"><div className="volume-control"><Volume2 size={17}/><Slider aria-label="音量" value={[volume]} min={0} max={1} step={.05} onValueChange={([v])=>{setVolume(v);if(audioRef.current)audioRef.current.volume=v;}}/></div><a href={current.url} target="_blank" rel="noreferrer">听完整首<ArrowUpRight size={15}/></a></div></>:<><div className="player-idle"><span className="idle-icon"><Headphones size={21}/></span><span>给下一首好歌，<strong>一点时间。</strong></span></div><button className="idle-play" onClick={()=>play(focused||featured[0])}>{previewOnly?"去平台听":"开始试听"}<Play size={15} fill="currentColor"/></button><span className="player-caption">{previewOnly?"去音乐平台，听完整首。":"约 30 秒，先听一点喜欢。"}</span></>}</div>
